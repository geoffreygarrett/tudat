...
#ifndef TUDAT_HYBRIDARCSIMULATOR_H
#define TUDAT_HYBRIDARCSIMULATOR_H
...
struct HybridArcState : public SimulatorState<F, T> {};

/// @tparam F the state floating-point type (e.g. float, double).
/// @tparam T the domain type being integrated over (e.g. time).
/// @tparam H the domain step-size type.
template <typename F = double, typename T = double, typename H = double>
class HybridArcSimulator : public BaseSimulator<F, T> {
public:
 using DynamicsSimulator<F, T>::bodies_;
 using DynamicsSimulator<F, T>::clearNumericalSolutions_;
  ...
  *        multi arc type)
  * @param arcStartTimes Times at which
  *        the separate arcs start, for the multi-arc case
  ...
  *        (default true).
  * @param addSingleArcBodiesToMultiArcDynamics Boolean
  *        denoting whether to add single arc bodies to multi-arc dynamics
  * (default false).
  */
 HybridArcSimulator(
     ...
     : BaseSimulator<S, T>(bodies, clearNumericalSolutions,
                           setIntegratedResult) {
   std::shared_ptr<HybridArcPropagatorSettings<S>> hybridPropagatorSettings =
       std::dynamic_pointer_cast<HybridArcPropagatorSettings<S>>(
           propagatorSettings);
   if (hybridPropagatorSettings == nullptr) {
     throw std::runtime_error(
         "Error when making HybridArcDynamicsSimulator, propagator settings "
         "are incompatible");
   }

   singleArcDynamicsSize_ =
       hybridPropagatorSettings->getSingleArcPropagatorSettings()
           ->getPropagatedStateSize();
   multiArcDynamicsSize_ =
       hybridPropagatorSettings->getMultiArcPropagatorSettings()
           ->getPropagatedStateSize();

   if (!addSingleArcBodiesToMultiArcDynamics) {
     if (!setIntegratedResult) {
       std::cerr << "Warning in hybrid dynamics simulator, "
                    "setIntegratedResult is false, but single arc propagation "
                    "result will be set in environment for consistency with "
                    "multi-arc "
                 << std::endl;
     }
     singleArcDynamicsSimulator_ = std::make_shared<
         SingleArcDynamicsSimulator<StateScalarType, TimeType>>(
         bodies, integratorSettings,
         hybridPropagatorSettings->getSingleArcPropagatorSettings(), false,
         false, true);
     multiArcDynamicsSimulator_ = std::make_shared<
         MultiArcDynamicsSimulator<StateScalarType, TimeType>>(
         bodies, integratorSettings,
         hybridPropagatorSettings->getMultiArcPropagatorSettings(),
         arcStartTimes, false, false, setIntegratedResult);
   } else {
     throw std::runtime_error(
         "Cannot yet add single-arc bodies to multi-arc propagation");
   }
 ...
 HybridArcDynamicsSimulator(
     const SystemOfBodies &bodies,
     const std::shared_ptr<IntegratorSettings<T>> singleArcIntegratorSettings,
     const std::shared_ptr<IntegratorSettings<T>> multiArcIntegratorSettings,
     const std::shared_ptr<PropagatorSettings<F>> propagatorSettings,
     const std::vector<double> arcStartTimes,
     const bool areEquationsOfMotionToBeIntegrated = true,
     const bool clearNumericalSolutions = true,
   ...
   std::shared_ptr<HybridArcPropagatorSettings<StateScalarType>>
       hybridPropagatorSettings = std::dynamic_pointer_cast<
           HybridArcPropagatorSettings<StateScalarType>>(propagatorSettings);
   if (hybridPropagatorSettings == nullptr) {
     throw std::runtime_error(
         "Error when making HybridArcDynamicsSimulator, propagator settings "
         "are incompatible");
  ...
  * Encke, Gauss, etc. for translational dynamics). The states for all arcs
  * must be concatenated in order into a single Eigen Vector, starting with the
  * single-arc states, followed by the mulit-arc states
  */
 void integrateEquationsOfMotion(
   ...
   multiArcDynamicsSimulator_->integrateEquationsOfMotion(
       initialGlobalStates.block(singleArcDynamicsSize_, 0,
                                 multiArcDynamicsSize_, 1));
 }
 ...
 /*!
  *  This function updates the environment with the numerical solution of the
  * propagation (no additional functionality in hybrid arc). Function may be
  * used to process manually updtaed propagation results in single and/or
  * multi-arc model
  */
  ...
  */
 std::shared_ptr<MultiArcDynamicsSimulator<StateScalarType, TimeType>>
 getMultiArcDynamicsSimulator() {
 ...
 virtual bool integrationCompletedSuccessfully() const {
   return !(singleArcDynamicsSimulator_->integrationCompletedSuccessfully() ==
                false ||
            multiArcDynamicsSimulator_->integrationCompletedSuccessfully() ==
                false);
 }
  ...
  * last numerical integration. First vector entry contains single-arc results.
  * Each subsequent vector entry contains one of the multi-arcs. Key of map
 ...
 getEquationsOfMotionNumericalSolutionBase() {
   std::vector<
       std::map<TimeType, Eigen::Matrix<StateScalarType, Eigen::Dynamic, 1>>>
       numericalSolution = singleArcDynamicsSimulator_
                               ->getEquationsOfMotionNumericalSolutionBase();
   std::vector<
       std::map<TimeType, Eigen::Matrix<StateScalarType, Eigen::Dynamic, 1>>>
       multiArcNumericalSolution =
           multiArcDynamicsSimulator_
               ->getEquationsOfMotionNumericalSolutionBase();
   numericalSolution.insert(numericalSolution.end(),
                            multiArcNumericalSolution.begin(),
                            multiArcNumericalSolution.end());

   return numericalSolution;
 }

 //! Function to return the numerical solution to the dependent variables (base
 //! class interface).
 /*!
  *  Function to return the numerical solution to the dependent variables for
  * last numerical integration. First vector entry contains single-arc results.
  * Each subsequent vector entry contains one of the multi-arcs. Key of map
  * denotes time, values are dependent variables vectors \return List of maps
 ...
 getDependentVariableNumericalSolutionBase() {
   std::vector<std::map<TimeType, Eigen::VectorXd>> numericalSolution =
       singleArcDynamicsSimulator_
           ->getDependentVariableNumericalSolutionBase();
   std::vector<std::map<TimeType, Eigen::VectorXd>> multiArcNumericalSolution =
       multiArcDynamicsSimulator_->getDependentVariableNumericalSolutionBase();
   numericalSolution.insert(numericalSolution.end(),
                            multiArcNumericalSolution.begin(),
                            multiArcNumericalSolution.end());
 ...
 /*!
  *  Function to return the map of cumulative computation time history that was
  * saved during numerical propagation.  First vector entry contains single-arc
  * results. Each subsequent vector entry contains one of the multi-arcs. Key
  * of map denotes time, values are computation times. \return Vector is size
  * 1, with entry: map of cumulative computation time history that was saved
  * during numerical propagation.
  */
 std::vector<std::map<TimeType, double>>
 getCumulativeComputationTimeHistoryBase() {
   std::vector<std::map<TimeType, double>> computationTime =
       singleArcDynamicsSimulator_->getCumulativeComputationTimeHistoryBase();
   std::vector<std::map<TimeType, double>> multiArcComputationTime =
       multiArcDynamicsSimulator_->getCumulativeComputationTimeHistoryBase();
   computationTime.insert(computationTime.end(),
                          multiArcComputationTime.begin(),
